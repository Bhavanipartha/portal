...
$today = date('Y-m-d');
$updateStatusSQL = "UPDATE project_allocation SET approved_status = 2 WHERE e_date < '$today'";
mysqli_query($connect, $updateStatusSQL); 

if (isset($_SESSION['uname']) && !empty($_SESSION['uname'])) {
    $uname = $_SESSION['uname'];
    
    $sql7 = "SELECT * FROM employee_details WHERE email = ?";
    ...
    if ($result7) {
        ...
        $sql1 = "SELECT name FROM employee_details WHERE status = 1 AND supervisor = ?";
        $stmt1 = $connect->prepare($sql1);
        $stmt1->bind_param("s", $name);
        $stmt1->execute();
        $resulti = $stmt1->get_result();
        ...
        if ($resulti) {
            ...
            while ($rowi = $resulti->fetch_assoc()) {
                ...
                $sql = "SELECT * FROM project_allocation WHERE status = 1 AND name = ? ORDER BY `s_date` DESC";
                $stmt = $connect->prepare($sql);
                $stmt->bind_param("s", $namei);
                $stmt->execute();
